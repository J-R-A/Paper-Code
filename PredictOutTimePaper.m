function [Laps, dummyLaps, allPredMat, allTrials, trialEvents, allDists, coefs, fitInfo] = PredictOutTimePaper(pMats,trialInfo,trialEvents,Distances,elnetVal)
...
for d=del
    ...
    out=trialInfo{1,d}(:,1); % Target Speeds
    
    
    % just saving the actual data being used for sanity check...
    allPredMat{1,d}=predictorsMatrix;
    allTrials{1,d} = trialInfo{1,d};
    allDists{1,d} = Distances{1,d};
    ...
    for icv=1:nOutFolds
        ...
        cvFitSpeeds=cvglmnet(predictorsMatrix(iTrain,:),out(iTrain,:),'binomial',options,[],nfolds,[],0); % Fits the model parameters and hyperparameters using "nfolds" cross validation
    ...
    cvFitBothAll=cvglmnet(predictorsMatrix,out,'binomial',options); % run cross val to find optimal lambda
