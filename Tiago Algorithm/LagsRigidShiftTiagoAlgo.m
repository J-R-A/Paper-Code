...
[matSpikesLag] = SmoothRigid(tSpikes,stdv,tTms,tTmsSpk,sampling,lags(9,:)');
[matSpikesNoLag] = SmoothRigid(tSpikes,stdv,tTms,tTmsSpk,sampling,zeros(length(lags(1,:)),1));
[matSpikesDummy] = SmoothRigid(tSpikes,stdv,tTms,tTmsSpk,sampling,ones(length(lags(1,:)),1)*0.005);



PredsLag = matSpikesLag * coefs(10,:)' ;
PredsNoLag = matSpikesNoLag * coefs(1,:)';
