...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 29 01:25:38 2020

@author: tjpc
"""
# PLOT AUTO CORRELATION OF SPEED!!!!!
...
all_data = pickle.load( open(path + sessions[2], "rb" ))
##############################################################################################
# organizing data 
...
n_iter = 5                                  # number of cycles computing the betas and the lag function


lrate = 0.01                                # Learning rate for the ES algorithm       
...
reg_smoothess = 0.005                       # regularization parameter of the lag function5
...
lag_vectors = np.zeros((n_iter, NBINS + 1))
regressors_vectors = np.zeros((n_iter,96)) 
neural_signals = np.zeros((n_iter,NBINS+1,24,96))



lag_vector = np.zeros(NBINS+1)
...
for i in range(n_iter):
    ...
    # print(lag_vector)
    X_reg, X, time = get_data_matrix(sorted_spiketimes_windowed,sorted_neurons_id_windowed,
                                     sorted_trial_id_windowed,TMIN,TMAX,lag_vector,sigma_conv, 
                                     BINSIZE, type_kernel = 'real')
    
    fit = regression_model.fit(X_reg.copy(), y.copy())
    regressors = fit.coef_
    
    initial_lag_vector = np.zeros(NBINS+1)
    # replace pred_y with the y (the real speed) from the previous section, in the compute_optimal_lag_ES function.
    
    
    lag_vector = compute_optimal_lag_ES(pred_y, initial_lag_vector, regressors, sorted_spiketimes_windowed, 
                                        sorted_neurons_id_windowed,sorted_trial_id_windowed, 
                                        TMIN, TMAX, sigma_conv, BINSIZE, lrate = lrate,
    ...
    neural_signals[i,:,:,:] = X.copy()


print('time it took: ', str(timeit.default_timer()-start))
