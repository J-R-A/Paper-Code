function [SpeedMat,FeatMat,tInfoMat,tEvMat,distsMat,paramInfo] = MakeFeaturesMatrixOutPredictPaper(Name,event,intCond,stdv,spdSmt,machinePath)
...
% Mix Trials from both coditions
...
Features{1,end+1} = vertcat(Features{:});
Features{1,end} = Features{1,end}(trialsOrder,:); 

Speeds{1,end+1} = vertcat(Speeds{:});
Speeds{1,end} = Speeds{1,end}(trialsOrder,:); 

trialInfo{1,end+1} = vertcat(trialInfo{:});
trialInfo{1,end} = trialInfo{1,end}(trialsOrder,:);

trialEvents{1,end+1} = vertcat(trialEvents{:});
trialEvents{1,end} = trialEvents{1,end}(trialsOrder,:);

Distances{1,end+1} = vertcat(Distances{:});
...
for d = 1:length(Features(1,:))
    ...
    if length(t1) > length(t2)
        ...
        grouped = [t1(1:length(t2))'; t2'];
        holdOut = t1(length(t2)+1:end);
        combined = reshape(grouped,numel(grouped),1);
        
        for t = 1:length(holdOut)
            tIdx = randi(length(combined));
            combined = [combined(1:tIdx-1); holdOut(t); combined(tIdx:end)];
        end
        
         tOrder = combined;
    else
        
        grouped = [t1'; t2(1:length(t1))'];
        holdOut = t2(length(t1)+1:end);
        combined = reshape(grouped,numel(grouped),1);
    ...
    Features{1,d} = Features{1,d}(tOrder,:);
    Speeds{1,d} = Speeds{1,d}(tOrder,1);
    trialInfo{1,d} =  trialInfo{1,d}(tOrder,1);
    trialEvents{1,d} = trialEvents{1,d}(tOrder,1);
    Distances{1,d} = Distances{1,d}(tOrder,1);
...
for d=1:length(trialInfo(1,:))
        ...
        filter1 = tInfoMat{1,d}(:,1) == rwddSound;
        filter2 = tInfoMat{1,d}(:,1) == nRwddSound;
        tInfoMat{1,d}(filter1,1) = 1;
        tInfoMat{1,d}(filter2,1) = 0;
